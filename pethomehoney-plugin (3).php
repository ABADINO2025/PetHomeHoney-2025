...
// Asegurarse de que los archivos requeridos solo se carguen una vez y en el contexto de administración.
if (is_admin()) {
...
if (is_admin()) {
    ...
    /**
     * Procesa el formulario de la página "Agregar Guarda" (OBSOLETO, PERO SE MANTIENE POR RETROCOMPATIBILIDAD).
     */
    function pethome_guardas_save_callback() {
        ...
        // Redirige a la nueva pantalla de edición del post recién creado
    ...
    function pethome_register_admin_menus() {
        // 1. Crea el menú principal (padre).
        add_menu_page(
            'Guardería de Mascotas',
            'Guardería de Mascotas',
            'manage_options',
            'pethome_main',
            'pethome_main_page_callback',
            'dashicons-pets',
            56
        );

        // 2. Restaura el 'Panel Principal' como primer submenú.
        add_submenu_page(
            'pethome_main',
            'Panel Principal',
            'Panel Principal',
        ...
        // 3. El resto de los submenús personalizados.
    ...
    function pethome_guardas_agregar_page_callback() {
        // Esta página ya no se usa, redirigimos a la nueva pantalla nativa.
        wp_safe_redirect(admin_url('post-new.php?post_type=reserva_guarda'));
        exit;
    }
...
function pethome_global_admin_styles() {
    echo '<style>
        ...
        /* --- CÓDIGO NUEVO PARA ARREGLAR FLATPICKR --- */
        .flatpickr-calendar {
            z-index: 9999 !important; /* Un z-index muy alto para que aparezca por encima de todo */
        }
...
https://pethomehoney.com.ar/wp-admin/admin.php?page=wpide#            .pethome-resumen-metabox .resumen-details p i { margin-right: 8px; color: #5e4365; width: 16px; text-align: center; }
    ...
    add_action('admin_enqueue_scripts', function ($hook) {
        // Carga global de Font Awesome en todo el admin.
        ...
        // Verificamos si estamos en la pantalla de edición o creación del CPT 'reserva_guarda'.
        if ($screen && $screen->post_type === 'reserva_guarda') {
            
            // Carga de la librería Flatpickr
            ...
            // Carga de nuestro archivo JS personalizado para el CPT
            ...
            wp_enqueue_script(
                'pethome-reserva-guarda-js', 
                plugin_dir_url(__FILE__) . 'assets/js/pethome_admin_reserva_guarda.js', 
                ['jquery', 'flatpickr-js'], 
                $custom_js_version, 
                true
            );
        ...
        // Carga de estilos y scripts para otras páginas específicas (como la de fusionar clientes).
        if ($screen && $screen->id === 'guarder-a-de-mascotas_page_pethome_clientes_fusionar') {
        ...
        // Estas dos líneas pueden permanecer si son necesarias en varias páginas.
        wp_enqueue_media();
    ...
    function pethomehoney_reserva_details_cb($post){
        ...
        $fields_config = [
            'Reserva' => ['icon' => 'fa-thin fa-calendar-check', 'items' => [
                ...
                'pethome_reserva_fechas' => ['label' => 'Fechas Reservadas', 'type' => 'text', 'id' => 'pethome_reserva_fechas'],
                'pethome_reserva_cantidad_dias' => ['label' => 'Cantidad de Días', 'type' => 'number', 'id' => 'pethome_reserva_cantidad_dias'],
    ...
    function pethome_registrar_widget_escritorio() {
        ...
        if (isset($wp_meta_boxes['dashboard']['normal']['core']['pethome_resumen_widget'])) {
    ...
    function pethome_render_widget_escritorio() {
        ...
        $cuidadores_option = get_option('pethome_cuidadores', []);
        $cuidadores = is_array($cuidadores_option) ? $cuidadores_option : [];
        $total_cuidadores = count($cuidadores);

        // -- Preparación para Gráficos y Top 10 --
        $cuidador_keys = array_map(function($c) { return $c['alias'] ?? null; }, $cuidadores);
        $cuidador_counts = array_fill_keys(array_filter($cuidador_keys), 0);
        $months = [];
        ...
        if ($all_items_query->have_posts()) {
            while ($all_items_query->have_posts()) {
                ...
                if (isset($months[$item_date_key])) {
                    ...
                    $monto = 0;
                    if ($item_type === 'wc_booking' && class_exists('WC_Booking')) {
        ...
        <style>
            ...
            .chart-wrapper { height: 200px; margin-bottom: 25px; }
            ...
            .phh-latest-list, .phh-top-list { list-style: none; margin: 0; padding: 0; }
        ...
        <div class="phh-widget-container">
            <div class="phh-widget-grid">
                ...
                <a href="<?php echo admin_url('edit.php?post_type=reserva_guarda'); ?>" class="phh-stat-card">
                    ...
                    <div>
                        ...
                        <?php if (!empty($top_cuidadores)) : ?>
                            <ol class="phh-top-list">
                                <?php $pos = 1; foreach ($top_cuidadores as $cuidador_alias => $count) : ?>
                                    <li>
                                        <span class="pos"><?php echo $pos++; ?>.</span>
                                        <span class="name"><?php echo esc_html($cuidador_alias); ?></span>
                                ...
                                <li>
                                    <a href="<?php echo get_edit_post_link(get_the_ID()); ?>">
                                        ...
                                        <div class="item-details">
                                            ...
                                            <?php 
                                                $nombre_mascota = 'N/D';
                                                if (get_post_type() === 'wc_booking' && class_exists('WC_Booking')) {
            ...
            <div class="phh-section">
                ...
                <div class="phh-links-grid">
                    <a href="<?php echo admin_url('post-new.php?post_type=reserva_guarda'); ?>" class="phh-button add-new"><i class="fa-thin fa-plus"></i><span>Nueva Reserva</span></a>
                    <a href="<?php echo admin_url('edit.php?post_type=reserva_guarda'); ?>" class="phh-button"><i class="fa-thin fa-list-ul"></i><span>Ver Reservas</span></a>
